  ...
  while [ "$IN" != "$iter" ] ;do
    ...
    if [[ -z "${module_name/ }" ]]; then
      if [[ -z "${modules_list/ }" ]]; then
        tmux_echo "tmux_theme warning: a module list has only white space, to remove all modules set it to \\\"null\\\"" 100
      else
        tmux_echo "tmux_theme warning: a module list with value \\\"$modules_list\\\" has leading/trailing whitespace" 101
      fi
      continue
    fi

    tmux_echo "tmux_theme warning: module $module_name not found" 102
